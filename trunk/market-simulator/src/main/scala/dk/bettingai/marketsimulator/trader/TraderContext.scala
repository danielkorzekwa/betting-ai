...
class TraderContext(nextBetId: => Long, val userId: Int, market: IMarket, val commission: Double, val bank: Double, simulator: Simulator, marketSimActor: MarketSimActor) extends ITraderContext {
   ...
   * @param listener
   */
  def addUnmatchedBetsListener(filter: (IBet) => Boolean, listener: (IBet) => Unit) = market.addUnmatchedBetsListener(filter,listener)
   ...
   * @param listener
   */
  def addCancelledBetsListener(listener: (IBet) => Unit) = market.addCancelledBetsListener(listener)
}
