...
class PricePriceSlopeTraderRandomTest {
  ...
  val baseTrader = PricePriceSlopeTrader("baseTrader", -0.21, 0.21, 5,20)
  ...
  def testTwoMarkets {
   ...
   //   val marketDataDir = "c:/daniel/marketdatafull"
    ...
    val mutate = (solution: Solution[PricePriceSlopeTrader]) => {
      ...
      val trader = PricePriceSlopeTrader("trader" + nextTraderId, backPriceSlopeSignal, layPriceSlopeSignal, maxPrice,maxNumOfRunners)
      trader
    }
