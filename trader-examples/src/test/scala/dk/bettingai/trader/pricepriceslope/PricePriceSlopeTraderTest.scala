...
class PricePriceSlopeTraderTest {
  ...
  val trader = PricePriceSlopeTrader("baseTrader", -0.21, 0.21, 5)
   ...
   // val marketDataDir = "c:/daniel/marketdata"
    ...
    val mutate = (solution: Solution[PricePriceSlopeTrader]) => {
      ...
      if (solution.matchedBetsNum > 0) {
        ...
        PricePriceSlopeTrader("trader" + nextTraderId, backPriceSlopeSignal, layPriceSlopeSignal, maxPrice)
      } else {
        ...
        val trader = PricePriceSlopeTrader("trader" + nextTraderId, backPriceSlopeSignal, layPriceSlopeSignal, maxPrice)
