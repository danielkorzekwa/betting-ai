...
object HillClimbing {
   ...
   *   @param quality Quality achieved by this trader.
   *   @param matchedBetsNumm Total number of matched bets for this trader.
   */
  case class Solution[T <: ITrader](trader: T, quality: Double, matchedBetsNum: Double) {
    override def toString = "Solution [trader=%s, quality=%s, matchedBetsNum=%s]".format(trader, quality, matchedBetsNum)
  }
...
trait HillClimbing[T <: ITrader] {
  ...
  def optimise(trader: T, generationNum: Int, progress: Option[Progress[T] => Unit] = None): Solution[T] = {
    ...
    val bestTrader = (1 to generationNum).foldLeft(initialSolution)((best, iter) => {
      ...
      val bestOfTwo = if (child.quality > best.quality) child else best
